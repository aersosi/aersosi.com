...
export class Carousel {
  ...
  constructor(carouselElem: HTMLElement | null) {
    ...
    this.imagesElements = Array.from(this.slidesUl.children).reduce<HTMLImageElement[]>((acc, c) => {
  ...
  private _changeCurrentSlide(oldIdx: number, newIdx: number) {
    ...
    if (this.circleNavigation) {
      ...
      const styles = ["!bg-primary-600", "scale-130"];
      circles.forEach((circle, i) => {
        if (i === newIdx) {
          circle.classList.add(...styles);
        } else {
          circle.classList.remove(...styles);
        }
      });
    }
  }
  ...
  private _addEvents() {
    ...
    if (this.circleNavigation) this.circleNavigation?.addEventListener("click", this._circleClickHandler);
